...
def sumup_db():
    ...
    print('* 연령대')
    print('- 20대 : %s명 (%0.1f%%)' % (len(age_20), (len(age_20) / count) * 100, age_20))
    print('- 30대 : %s명 (%0.1f%%)' % (len(age_30), (len(age_30) / count) * 100, age_30))
    print('- 40대 : %s명 (%0.1f%%)' % (len(age_40), (len(age_40) / count) * 100, age_40))
    print("")
...
def whole_db():
    ...
    mysql_set.execute("select * from student_info b left outer join student_languages s on b.student = s.student_id;")
    rows = mysql_set.fetchall()
    for row in rows:
        output = []
        for column_index in range(len(row)):
            output_full.append(str(row[column_index]))
        output_full.append(output)
...
def insert_db():
    ...
    while True:
        ...
        );""" %(table_list,ID,name_value,sex_value,age_value,major_value))
        for name in language_value:
            mysql_set.execute("""INSERT INTO %s VALUES ('%s', '%s', '%s', '%s', '%s');"""%
            table_language, ID, name, language_level[count], period_value[count])
            count += 1
...
def whole_search_db(num_data):
    ...
    mysql_set.execute(
        "select * from student_info b left outer join student_languages s using(student_id) where %s like '%%%s%%';"%(mode, search_data))
...
def update_db():
    ...
    try:
        mysql_set.execute(
            "select * from student_info b left outer join student_languages s on b.student_id = s.student_id='%s';" % update_id
        )
    except Exception:
        pass
    ...
    if update_info == '1':
        ...
        mysql_set.execute("update %s set sex='%s' where student_id='%s';" % (update_data, update_id))
    elif update_info == '2':
        mysql_set.execute("update %s set sex='%s' where student_id='%s';" % (update_data, update_id))
    elif update_info == '3':
        mysql_set.execute("update %s set age='%s' where student_id='%s';" % (update_data, update_id))
    elif update_info == '4':
        mysql_set.execute("update %s set major='%s' where student_id='%s';" % (update_data, update_id))
    if output_full[0][6] != 'None':
        count += 3
        count2 = 0
        num_of = 0
        ...
        for language_value in output_full:
            if (int(update_info) <= count+3 and int(update_info)>count):
                if int(update_info) % 3 == 2:
                    mysql_set.execute(
                        "update %s set name='%s' where student_id='%s'and name='%s';" % table_language, update_data, update_id, language_value[6]
                    )
                if int(update_info) % 3 == 0:
                    mysql_set.execute(
                        "update %s set level='%s' where student_id='%s'and name='%s';" % table_language, update_data, update_id, language_value[6]
                    )
                if int(update_info) % 3 == 1:
                    mysql_set.execute(
                        "update %s set period='%s' where student_id='%s'and name='%s';" % table_language, update_data, update_id, language_value[6]
                    )
            count += 3
    con.commit()
    time.sleep(0.5)
...
def insert_csv():
    try:
        ...
        mysql_set.execute(
            "CREATE TABLE IF NOT EXISTS student_info(Student_ID VARCHAR(20) ,Name VARCHAR(20) ,sex VARCHAR(20) ,Age int, Major VARCHAR(20));")
        con.commit()
    try:
        mysql_set.execute("desc %s" % table_language)
    except Exception:
        mysql_set.execute(
            "CREATE TABLE IF NOT EXISTS student_languages(Student_ID VARCHAR(20), Name VARCHAR(20), level VARCHAR(20), period VARCHAR(20));"
        )

    with open('student_info.csv', newline='', encoding='utf-8') as infile:
        data = list(csv.reader(infile))
    with open('student_language.csv', newline='', encoding='utf-8') as infile:
        data2 = list(csv.reader(infile))

    sel_op = input("기존 데이터가 다 사라지고 csv파일의 데이터를 불러옵니다. 하시겠습니까(y/n): ")
    if sel_op == 'y':
