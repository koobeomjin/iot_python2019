    ...
    if input_file.split('.')[1] == 'csv':
        ...
        for worksheet in workbook.sheets():
            ...
            for row in range(1, worksheet.nrows):
                ...
                if str(worksheet.cell(row,0)).split('.')[0].strip() in item_numbers_to_find:
                    filewriter.writerow(row_of_output)
                    count_of_item_numbers += 1
                line_counter += 1
        print('Numner of files: {}'.format(file_counter))
        print('Numner of lines: {}'.format(line_counter))
        print('Numner of item numbers: {}'.format(count_of_item_numbers))