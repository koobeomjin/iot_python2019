...
print(note.get('method')) # 없는 속성에 대하여 None을 반환한다.
print(note.get('method','gamil')) # 없는 속성값에 대한 default 값을 지정한다.
note.attrib['method'] = 'gmail' # 실제 속성 및 값 추가

print(note.keys()) # 해당 노드의 모든 속성 이름을 리스트로 반환
print(note.items()) # 해당 노드의 모든 속성과 값을 튜플형태로 반환
