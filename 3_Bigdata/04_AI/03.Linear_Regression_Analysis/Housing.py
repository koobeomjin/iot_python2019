# 총 조합 갯수 = 2047
# 5% MAX 조합: lotsize + bathrms + stories + driveway01 +
# fullbase01 + garagepl + prefarea01  >> 22.81 %
# 10% MAX 조합: lotsize + bathrms + stories + fullbase01 +
# airco01 + garagepl + prefarea01  >> 40.16 %
# 20% MAX 조합: lotsize + bathrms + stories + driveway01 +
# recroom01 + gashw01 + airco01 + prefarea01  >> 68.03 %
# 걸린시간:  0:02:15.976500
...
for num in range(1,12):
    ...
    for tup in combi_list:
        ...
        for index in range(len(y_predicted)):
            ### 5%
            if delta_minus(house['price'][index], 0.05) < y_predicted[index] < \
            delta_plus(house['price'][index], 0.05):
                match_count_5 += 1
            ### 10%
            if delta_minus(house['price'][index], 0.1) < y_predicted[index] < \
                    delta_plus(house['price'][index], 0.1):
                match_count_10 += 1
            ### 20%
            if delta_minus(house['price'][index], 0.2) < y_predicted[index] < \
                    delta_plus(house['price'][index], 0.2):
                match_count_20 += 1
